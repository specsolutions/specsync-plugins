...
public class ScenarioOutlinePerExamplesParser(ITagServices tagServices) : GherkinSourceDocumentParser(tagServices)
{
    ...
    private bool IsSpecialScenarioOutline(ScenarioOutline scenarioOutline, ScenarioLocalTestCase testCase)
    {
        ...
        if (scenarioOutlineExamples.Length > 1)
            return true;
        if (scenarioOutlineExamples.Length == 0 || testCase.IdLink != null)
            return false;
        var scenarioOutlineLocalTestCaseTags = scenarioOutlineExamples[0].Tags
            .Select(t => new LocalArtifactTag(t.Name.TrimStart('@')))
            .ToArray();
        return _tagServices.GetTestCaseLinkFromTags(scenarioOutlineLocalTestCaseTags) != null;
    }

    protected override IEnumerable<ScenarioLocalTestCase> CreateLocalTestCases(GherkinDocument gherkinDocument, EditableCodeFile codeFile, SourceDocumentParserArgs args)
    {
    ...
    private ExamplesLocalTestCase CreateLocalTestCaseForExamples(GherkinDocument gherkinDocument, Examples examples, int examplesIndex, ScenarioOutline scenario, Rule? rule, EditableCodeFile codeFile, SourceDocumentParserArgs args)
    {
        try
        {
    ...
    protected override FeatureFileUpdater CreateUpdater(EditableCodeFile codeFile, GherkinLocalTestCaseFormatter formatter, SourceDocumentParserArgs args)
    {
